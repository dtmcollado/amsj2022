...
BEGIN
	...
	WHERE m.date  >= _fecha_desde::timestamp without time zone AND m.date <= _fecha_hasta::timestamp without time zone
	    AND m.price_unit > 0::double precision
    	AND m.product_qty > 0::double precision
   ...
   CREATE INDEX ON temp_stock_quant_move_rel_salidas (move_id);
    ...
    SELECT product_tmpl_id, price_unit as precio_ultima_compra , name as proveedor
    FROM (
	        ...
	        INNER JOIN (
	            ...
	            FROM account_invoice ai
	            INNER JOIN account_invoice_line ail ON ai.id = ail.invoice_id
	            WHERE ai.state = 'open' or ai.state = 'paid'
	        	AND date_invoice >= _fecha_desde::timestamp without time zone AND date_invoice <= _fecha_hasta::timestamp without time zone
	        ...
	        SELECT pt.id AS product_tmpl_id, "ir_property"."value_float"::numeric AS precio_producto, '-- sin compra --' as name
	        FROM "ir_property"
	        INNER JOIN "ir_model_fields" ON ("ir_model_fields"."id" = "ir_property"."fields_id")
	        INNER JOIN product_template pt ON concat('product.template,',pt.id::character varying) = "ir_property"."res_id"
	        WHERE "ir_model_fields"."name"='standard_price' AND "ir_model_fields"."model"='product.template'
	        AND pt.id not in (
	            SELECT distinct p.product_tmpl_id
	            FROM account_invoice ai
	            INNER JOIN account_invoice_line ail ON ai.id = ail.invoice_id
	            INNER JOIN product_product p ON p.id = ail.product_id
	            WHERE ai.state = 'open' or ai.state = 'paid'
    ...
    SELECT * FROM stock_move m
	WHERE m.date  >= _fecha_desde::timestamp without time zone AND m.date <= _fecha_hasta::timestamp
	    AND m.price_unit > 0::double precision
    	AND m.product_qty > 0::double precision
...
RETURN query
 ...
 SELECT c.name AS sector,
    ...
    case when coalesce(quant.cost,0) > 0 then quant.cost else m.price_unit end AS costo,
    ...
    0 AS centro_costos,
    case when coalesce(quant.cost,0) > 0 then quant.cost else m.price_unit end AS cost,
    ...
    m.product_qty * (case when coalesce(quant.cost,0) > 0 then quant.cost else m.price_unit end)::numeric(18,2)::double precision AS total_fifo,
    m.product_qty * ip.precio_ultima_compra::numeric(18,2)::double precision AS total_ultima_compra,
    ...
    (case when coalesce(quant.cost,0) > 0 then quant.cost else m.price_unit end)::numeric(18,2)::double precision AS ValorFIFOUnitario,
    ip.precio_ultima_compra::numeric(18,2)::double precision AS ValorUltCompraUnit,
    ...
    ru.rubro , ru.codigo , ro.rubro as rubro_consumo , ro.codigo as codigo_consumo
   FROM temp_stock_move_salidas m
     ...
     INNER JOIN temp_facturas AS ip ON ip.product_tmpl_id = p.product_tmpl_id
 ...
 SELECT c.name AS sector,
    ...
    sub.name AS subgrupo,
    forma.name AS forma_farmaceutica,
    (case when coalesce(quant.cost,0) > 0 then quant.cost else m.price_unit end ) * -1 AS costo,
    replace(l.complete_name::text, 'Physical Locations / '::text, ''::text)::character varying AS origen,
    replace(l2.complete_name::text, 'Physical Locations / '::text, ''::text)::character varying AS destino,
    0 AS centro_costos,
    (case when coalesce(quant.cost,0) > 0 then quant.cost else m.price_unit end) * -1 as cost,
    ...
    m.location_id AS origen_id,
    p.default_code AS codmsp,
    m.product_qty * -1 * (case when coalesce(quant.cost,0) > 0 then quant.cost else m.price_unit end)::numeric(18,2)::double precision AS total_fifo,
    m.product_qty * -1 * ip.precio_ultima_compra::numeric(18,2)::double precision AS total_ultima_compra,
    almacen_entrada.name AS almacen_origen,
    almacen_salida.name AS almacen_destino,
    l.id,
    centro_entrada.name AS rubro_centro_entrada,
    centro_salida.name AS rubro_centro_salida,
    ((case when coalesce(quant.cost,0) > 0 then quant.cost else m.price_unit end) * -1)::numeric(18,2)::double precision AS ValorFIFOUnitario,
    (ip.precio_ultima_compra * -1)::numeric(18,2)::double precision AS ValorUltCompraUnit,
    usuario.name as usuario,
    centro_entrada.rubro AS rubro_entrada,
    centro_salida.rubro AS rubro_salida,
    ip.proveedor,
  ...
  WHERE l2.usage::text = 'internal'::text
    AND ((CASE
