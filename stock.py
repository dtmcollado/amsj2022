...
class stock_picking(osv.osv):
    ...
    def _create_backorder(self, cr, uid, picking, backorder_moves=[], context=None):
        ...
        """
        #agregado pedro### enla ultima linea cambia fecha del sigueinte picking
        if picking:
            picking.date=datetime.now()
        ######
        ...
        if backorder_move_ids:
            ...
            ##### Agregado pedro
            backorder.date=datetime.now()
            ######
            
            #agregado Cesar la idea es dejar la Entrega Parcial en Borrador
            picking_borrador=self.do_unreserve(cr, uid, [backorder_id], context=context)
            ####

            #agregado Pedro para cambiar el estado de los quants
            move_lines_ids = [move.id for move in backorder.move_lines]
            
            quant_obj = self.pool.get("stock.quant")
            mov_obj = self.pool.get("stock.move")

            for mv in move_lines_ids:
            ...
            #print 'backorder',backorder,'backorder_id', backorder_id, 'Picking', picking,'backorder.move_lines',move_lines_ids
    ...
    def process_barcode_from_ui(self, cr, uid, picking_id, barcode_str, visible_op_ids, context=None):
        ...
        if matching_product_ids:
            # Fernando Clavijo
            # Se verifica que el producto estè en el pedido
            product_id = matching_product_ids[0]

            #Modificado por Pedro, SOLO la siguiente linea--->
            if stock_operation_obj.search(cr, uid, [("product_id", "=", product_id),("picking_id","=",picking_id)]):
                # Agregar lìnea sòlo si este producto pertenece a la orden de compra
...
class stock_pack_operation(osv.osv):
    ...
    def create_and_assign_lot(self, cr, uid, id, name, context=None):
        ...
        expirate_date = context.get("expirate_date", False)
        try:
            if expirate_date != False:
                alert_date = datetime.strptime(expirate_date, "%Y-%m-%d") - relativedelta.relativedelta(days=60)
                val.update({
                    "life_date": expirate_date + " 10:00:00",
                    "alert_date": alert_date.strftime("%Y-%m-%d") + " 10:00:00",
                })
        except:
            pass
            # now = date.today()
    ...
    def _search_and_increment(self, cr, uid, picking_id, domain, filter_visible=False, visible_op_ids=False, increment=True, context=None):
        ...
        if todo_operation_ids:
            ...
            if increment:
                for _operation_id in todo_operation_ids:
                    # Si uso el lector, busco la primera linea que no esté completa
                    # Si apreto el botón de más y ya está completa, no altera
                    # ninguna otra linea
                    op_obj = self.browse(cr, uid, _operation_id, context=context)
                    if op_obj.qty_done < op_obj.product_qty:
                    # if op_obj.qty_done == 0:
            ...
            if operation_id is not None:
                ...
                # Fernando Clavijo
                # Esto es mucho mas rapido que hacer self.write
                sql = """UPDATE stock_pack_operation set qty_done = %(qty)s WHERE id = %(operation_id)s"""
