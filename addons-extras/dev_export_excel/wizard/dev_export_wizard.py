        ...
        for field in field_val:
            if len(field) > 1:
                ...
                if field_val:
                    ...
                    main_vals.append(final_val or '')
                else:
                    main_vals.append(' ')
            else:
                model_id_val = model_id.read([field[0]])
                field_val = model_id_val[0].get(field[0])
                main_vals.append(field_val or '')
        return main_vals
    ...
    def export_excel(self):
        ...
        if rec.is_group_by:
            ...
            for model in obj_ids:
                vals= model.read(group_field_name)
                group_field_vals.append(vals[0])
                ...
                if rec.sub_group_by_field_id:
                    ...
                    for sub_result in sub_results:
                        ...
                        for val in sub_result['values']:
                            col=0
                            for g_field in group_field_name:
                                if isinstance(val[g_field], tuple):
                                    if isinstance(val[g_field][1], float) or isinstance(val[g_field][1], int):
                                        main_val_format = workbook.add_format()
                                        main_val_format = self.set_main_val_format(rec,main_val_format,'right')
                                        worksheet[0].write(row,col, format((val[g_field][1]),'.2f') or 0.00,main_val_format)
                                    else:
                                        worksheet[0].write(row,col, val[g_field][1] or ' ',main_val_format)
                                    main_val_format = workbook.add_format()
                                    main_val_format = self.set_main_val_format(rec,main_val_format)
                                else:
