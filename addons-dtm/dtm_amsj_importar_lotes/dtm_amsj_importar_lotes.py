    ...
    def import_csv(self):
        ...
        for i in lineas:
            ...
            if len(mov)>=4 and no_titulo:
                ...
                if self.empaque:
                    # sql_prod = '''select id from product_product where default_code =  %(default_code)s and tipo_de_empaque = %(tipo_de_empaque)s'''
                    sql_prod = '''select p.id from product_product p inner join product_template pt on pt.id = p.product_tmpl_id where default_code =  %(default_code)s and pt.tipo_de_empaque = %(tipo_de_empaque)s'''
                    self.env.cr.execute(sql_prod, {'default_code': ref_interna, 'tipo_de_empaque':self.empaque.id})
                    sql_resultado = self.env.cr.fetchall()
                else:
                    sql_prod = '''select id from product_product where default_code =  %(default_code)s '''
                    self.env.cr.execute(sql_prod, {'default_code': ref_interna})
                    sql_resultado = self.env.cr.fetchall()
                ...
                if not producto:
                    ...
                    existe_prod = '''select id from product_product where default_code =  %(default_code)s '''
                    self.env.cr.execute(existe_prod, {'default_code': ref_interna})
                    resultado_existe = self.env.cr.fetchall()

                    if resultado_existe:
                        aux=('El siguiente producto no tienen el empaque seleccionado ',ref_interna, ' linea '+str(lineas_archivo), i)
                ...
                if producto and nro_lote and (codigo_amsj or self.ubicacion):
                    #print ref_interna, lineas_archivo
                    ...
                    sql = '''select id from stock_production_lot where name = %(nro_lote)s  
                                                                      AND product_id = %(p_id)s  '''
                    self.env.cr.execute(sql, {'nro_lote': nro_lote,'p_id': p_id}
                                             )
                    ...
                    #existe_stock_quant=self.env['stock.quant'].search([('product_id','=',p_id),('lot_id','=',saber_id_lot),('location_id','=',ubicacion_final)])

                    sql_stock = '''select id from stock_quant where product_id = %(pro)s and lot_id= %(lot)s and location_id= %(locat)s '''
                    self.env.cr.execute(sql_stock, {'pro': p_id, 'lot':saber_id_lot, 'locat':ubicacion_final})
                    existe_stock_quant = self.env.cr.fetchall()


                    if existe_stock_quant:
                        cantidad_registros+=1
                        existe_stock_quant=existe_stock_quant[0][0]
                        sql_quant='''UPDATE stock_quant 
                            set qty=%(cantidad)s
                            where id = %(existe)s; '''
                        self.env.cr.execute(sql_quant, {'existe':existe_stock_quant,'cantidad':cantidad})
                        #self.env['stock.quant'].browse(existe_stock_quant).write({'qty':cantidad})
                        update_archivos+=1


                    # if ubicacion_final != None and not existe_stock_quant:
                    elif ubicacion_final and not existe_stock_quant:
                        sql_stock_quant='''INSERT INTO stock_quant(product_id, qty, lot_id, location_id, in_date, cost, company_id) 
                                            VALUES(%(p_id)s, %(cantidad)s, %(saber_id_lot)s, %(ubicacion_final)s, %(hoy)s, %(costo_prod)s,1);'''


                        parametros = {
                             'p_id':p_id,
                             'cantidad':cantidad,
                             'saber_id_lot':saber_id_lot,
                             'ubicacion_final':ubicacion_final,
                             'hoy':hoy,
                             'costo_prod':costo_prod,}

                        self.env.cr.execute(sql_stock_quant,parametros)
                        cantidad_registros += 1
