...
class wizard_comprasPDF(models.TransientModel):
    _name = "wizard.compras.pdf"
    ...
    tipo_facturas = fields.Selection([('autorizadas', 'TODAS las facturas autorizadas para pagar en el período que se está filtrando el informe'),
                                      ('contable', 'TODAS las facturas autorizadas para pagar , por fecha contable')],
                                     string="Seleccionar tipo de Facturas", default='contable')
    ...
    def action_report(self):
        ...
        if self.tipo_facturas == 'autorizadas':

            data = {}
            data['ids'] = self._context.get('active_ids', [])
            data['model'] = self._context.get('active_model', 'ir.ui.menu')
            data['form'] = self.read()
            data['lineas'] = report_ops._datos_reporte_compras(self)

            return {
                'type': 'ir.actions.report.xml',
                'report_name': 'dtm_amsj_conusmo_de_productos.compras',
                'report_title': 'Compras',
                'datas': data,
            }
            ...
            data['model'] = self._context.get('active_model', 'ir.ui.menu')
            data['form'] = self.read()
            data['lineas'] = report_ops._datos_reporte_compras_contable(self)
