...
class dtm_amsj_export_fifo_puro(models.TransientModel):
    _name = "dtm.amsj.export.fifo.puro"

    ubicacion = fields.Many2one('stock.location', string='Ubicación', domain="[('usage','=','internal')]")
    tipo_resumen = fields.Selection(
        [('detallado', 'Fifo Detallado'), ('resumen', 'Fifo Resumen'), ('ubicacion', 'Stock por Ubicación')],
        string="Tipo de Reporte", default='detallado')
    opciones_productos = fields.Selection(
        [('0', 'Todos los Productos'), ('1', 'Productos inventariados en alguna ubicación'),
         ('2', 'Solo productos inventariados')], string="Productos a incluir", default='0')
    ...
    def action_export_validadas(self):
        ...
        if self.tipo_resumen == "detallado":
            sql = ''' 
                SELECT p.default_code::character varying as codigo,
                ...
                from sp_stock_history_fifo_lineas(%(fecha_balance)s,%(fecha_inicio)s, %(fecha_fin)s,%(param_ubic)s , %(param_prod)s , %(param_empaque)s,%(param_categ)s,%(param_invent)s,%(opciones_fecha)s ) as t
                INNER JOIN product_product p ON p.id = t.product_id
                left JOIN res_partner r on r.id = t.partner_id
            '''

            # titulos
            ws.write(0, 0, "Código", title_lime)
            ws.write(0, 1, 'Producto', title_lime)
            ws.write(0, 2, 'Cantidad', title_lime)
            ws.write(0, 3, 'Precio', title_lime)
            ws.write(0, 4, 'CJPU', title_lime)
            ws.write(0, 5, 'Total', title_lime)
            ws.write(0, 6, "Fecha de Factura Proveedor", title_lime)
            ws.write(0, 7, 'Factura', title_lime)
            ws.write(0, 8, 'Proveedor', title_lime)

            self.archivo_nombre = "Reporte_detallado_Fifo.xlsx"

        if self.tipo_resumen == "resumen":
            sql = ''' 
                SELECT p.default_code::character varying as codigo,
                    p.name_template::character varying as articulo,
                    quantity, total 
                from sp_stock_history_fifo_resumen(%(fecha_balance)s,%(fecha_inicio)s, %(fecha_fin)s,%(param_ubic)s , %(param_prod)s , %(param_empaque)s,%(param_categ)s,%(param_invent)s,%(opciones_fecha)s) as t
                INNER JOIN product_product p ON p.id = t.product_id
            ...
            ws.write(0, 1, 'Producto', title_lime)
            ws.write(0, 2, 'Cantidad', title_lime)
            ws.write(0, 3, 'Total', title_lime)
            # ws.write(0,3,'Fecha de Vencimiento',title_lime)
            # ws.write(0,4,'Total',title_lime)
            self.archivo_nombre = "Reporte_resumen_Fifo.xlsx"

        if self.tipo_resumen == "ubicacion":
            sql = ''' 
            ...
            ws.write(0, 4, 'Valor del Stock', title_lime)
            ws.write(0, 5, 'Categoría', title_lime)
        ...
        param_op_prod = int(self.opciones_productos)
        param_empaque = 1
        ...
        if len(resultado) > 0:
            ...
            for res in resultado:
                ...
                if self.tipo_resumen == "detallado":
                    ...
                    ws.write(fila, 6, self.modif_fecha(res[6]) if res[6] else '', estilo)
                    ws.write(fila, 7, res[7], estilo)
                    ws.write(fila, 8, res[8] if res[8] else '', estilo)
        ...
        return {
            ...
            'url': '/web/binary/download_document?model=dtm.amsj.export.fifo.puro&field=binario&id=%s&filename=%s' % (
                self.id,
                self.archivo_nombre,
            ),
            'target': 'self',
