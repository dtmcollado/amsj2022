...
class wizline(models.TransientModel):
    ...
    transf_id = fields.Many2one('wizard.envios')
...
class wizard(models.TransientModel):
    ...
    def get_domain_dest(self):
        ...
        # ])
        #
        # return [('id', '=', [wh.wh_output_stock_loc_id.id for wh in wharehouses])]

        wharehouses = self.env['stock.warehouse'].search([
            ('code', '=', 'SJM')
        ])

        return wharehouses.wh_input_stock_loc_id.id
    ...
    move_lines = fields.One2many('wizard.envios.line', inverse_name='transf_id', String='Internal Moves',
                                 copy=True)
    ...
    def saco_productos(self):
        lineas = self.env["wizard.envios.line"]
