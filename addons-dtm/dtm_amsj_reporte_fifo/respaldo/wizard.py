...
class wizard_reporte_fifo(models.TransientModel):
    ...
    def action_reporte_fifo(self):
        ...
        if not self.todos:
            ...
            ws.write(fila, 5, u"Precio total", header_left)
            fila += 1
            ...
            for location_id in stock_location_ids:
                for product in product_ids:
                    ...
                    if cantidad_stock > 0:
                        ...
                        fila += 1
                        product_category_obj = self.pool.get('product.category')
                        ws.write(fila, 0, product.default_code, header_left)
                        ws.write(fila, 1, product.name, header_left)
                        ws.write(fila, 2, nombre_almacen)
                        ws.write(fila, 3, product_category_obj.name_get(self._cr, self._uid,
                                                                            product.product_tmpl_id.categ_id.id)[0][
                                1])
                        ws.write(fila, 4, cantidad_stock, header_left)
                        fila += 1
                            # **************
                            # Escribo el titulo
                        ws.write(fila, 0, u"Fecha", header_left)
                        ...
                        self.env.cr.execute(
                            ...
                            (self.date, 'in_invoice', ('draft', 'cancel'),product.id,))
                        resultados = self.env.cr.fetchall()
                        cantidad_acumulado = float()
                        for resultado in resultados:
                            price = float()
                            ...
                            if cantidad_acumulado < cantidad_total_stock:
                                ...
                                ws.write(fila, 3, cantidad)
                                ws.write(fila, 4, abs(round(resultado[1],2)))
                                ws.write(fila, 5, price)
                                fila += 1
                        if cantidad_acumulado == 0:
                            price = round((product.product_tmpl_id.standard_price * cantidad_total_stock), 2)
                            precio_unitario = round(product.product_tmpl_id.standard_price,2)
                            ws.write(fila, 0, '24/07/2020')
                            ws.write(fila, 1, u'N/A')
                            ws.write(fila, 2, 'CARGA INICIAL')
                            ws.write(fila, 3, cantidad_total_stock)
        ...
        if not self.todos:
            ...
            for location_id in stock_location_ids:
                for product in product_ids:
                    ...
                    if cantidad_stock > 0:
                        self.env.cr.execute(
                            'SELECT AIL.QUANTITY,AIL.PRICE_UNIT,AI.DATE_INVOICE,AI.NUMBER,PARTNER.NAME FROM ACCOUNT_INVOICE AI,ACCOUNT_INVOICE_LINE AIL, PRODUCT_PRODUCT P,RES_PARTNER PARTNER WHERE AI.DATE_INVOICE <= %s AND AI.TYPE = %s AND AI.STATE NOT IN %s AND AIL.INVOICE_ID = AI.ID AND P.ID = AIL.PRODUCT_ID AND AI.PARTNER_ID = PARTNER.ID AND P.ID = %s ORDER BY AI.DATE_INVOICE DESC',
                            (self.date, 'in_invoice', ('draft', 'cancel'),product.id,))
                        resultados = self.env.cr.fetchall()
                        cantidad_acumulado = float()
                        price = float()
                        ...
                        if cantidad_acumulado> 0:
                            product_category_obj = self.pool.get('product.category')
                            ws.write(fila, 0, product.default_code or u'Sin código')
                            ...
                            fila += 1
                        if cantidad_acumulado == 0:
                            price = round((product.product_tmpl_id.standard_price * cantidad_stock), 2)
                            product_category_obj = self.pool.get('product.category')
                            ws.write(fila, 0, product.default_code or u'Sin código')
                            ws.write(fila, 1, product.name)
                            ws.write(fila, 2, product_category_obj.name_get(self._cr, self._uid,
                            ...
                            ws.write(fila, 3, nombre_almacen)
                            ws.write(fila, 4, cantidad_stock)
                            ws.write(fila, 5, abs(product.product_tmpl_id.standard_price))
                            ws.write(fila, 6, abs(price))
                            fila += 1
